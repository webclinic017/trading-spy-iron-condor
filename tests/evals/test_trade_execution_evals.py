...
class TradeExecutionEvals:
    ...
    def __init__(self, account_value: Decimal = Decimal("30000")):
        ...
        self.allowed_tickers = ["SPY", "SPX", "XSP", "QQQ", "IWM"]
    ...
    def eval_ticker(self, proposal: TradeProposal) -> EvalResult:
        """EVAL-001: Ticker must be in liquid ETF whitelist."""
        passed = proposal.ticker in self.allowed_tickers
        return EvalResult(
            passed=passed,
            rule="EVAL-001",
            message=f"Ticker {proposal.ticker} {'allowed' if passed else 'NOT allowed. Liquid ETFs only.'}",
        )
