...
def get_recent_trades(days: int = 14) -> list[dict]:
    """Get trades from system_state.json (synced from Alpaca) and daily files."""
    ...
    # Primary source: system_state.json (synced from Alpaca)
    system_state = load_json_file(DATA_DIR / "system_state.json")
    if isinstance(system_state, dict):
        trade_history = system_state.get("trade_history", [])
        for trade in trade_history:
            # Parse filled_at timestamp to get trade date
            filled_at = trade.get("filled_at", "")
            if filled_at:
                trade_date_str = filled_at[:10]  # Extract YYYY-MM-DD
                try:
                    trade_date = date.fromisoformat(trade_date_str)
                    if trade_date >= cutoff_date:
                        trade["trade_date"] = trade_date_str
                        trade["timestamp"] = filled_at
                        recent_trades.append(trade)
                except ValueError:
                    pass

    # Fallback: also check daily trade files
    for i in range(days):
        ...
        if trades_file.exists():
            ...
            if isinstance(day_trades, list):
                for trade in day_trades:
                    ...
                    if not any(
                        t.get("id") == trade.get("id") for t in recent_trades if t.get("id")
                    ):
                        recent_trades.append(trade)
